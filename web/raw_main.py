...
from adapter.ner_crf import adapterNerCrf
...
adapterNerCrf = adapterNerCrf.AdapterNerCrf()
...
def response_message():
    ...
    if intend == 1:
        ...
        for i in range(len(results['msg']['data'])):
            msg += " <strong> Tại " + str(results['msg']['data'][i]['địa điểm']).title() + " " + str(
                results['msg']['data'][i]['thời gian']) + ' :</strong> <br/>'
    ...
    if session['bot_state'] == 3:
        if data['LOC'] == [] and data['TIME'] == [] and data['WEATHER'] == []:
            ...
            if len(data["WEATHER"]) == 0:
                ...
                session['bot_state'] = 3
            session['bot_msg'] = query_api(
                {"loc": session['loc'], "time": session['time'], "weather": session['weather']})
